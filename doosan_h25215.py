...
urdf_path = "/Users/tommasoandina/Doosan_h2515/model.urdf"
# The joint list
...
class Simulator:
    ...
    def simulate_q(self, tau, M2, h2):
        dq = self.simulate_dq(tau, M2, h2)
        self.q += self.dt * dq
        return self.q

    def simulate_dq(self, tau, M2, h2):
...
for i in range(N):
    # Ricalcola M2 e h2 ad ogni iterazione
    M = kinDyn.mass_matrix_fun()
    M2 = cs.DM(M(H_b, simu.q))
    M2 = M2[6:, 6:]

    h = kinDyn.bias_force_fun()
    h2 = cs.DM(h(H_b, simu.q, v_b, simu.dq))
    h2 = h2[6:]

    tau = ctrl.control(simu.q, simu.dq)
    simu.simulate_q(tau, M2, h2)
    simu.simulate_dq(tau, M2, h2)
